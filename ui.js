document.addEventListener('DOMContentLoaded', () => {
    ...
    window.showLogoutConfirmModal = () => showModal(logoutConfirmModal, logoutConfirmBackdrop);
    window.hideLogoutConfirmModal = () => hideModal(logoutConfirmModal, logoutConfirmBackdrop);
    ...
    const showSidebar = () => {
        ...
        mainCard.classList.add('md:max-w-xl');
    ...
    const hideSidebar = () => {
        ...
        mainCard.classList.remove('md:max-w-xl');
    ...
    window.renderHistorySidebar = (history) => {
        ...
        if (history.length === 0) {
            historyList.innerHTML = '<p class="text-center text-sm text-muted-foreground p-4">No history yet.</p>';
            return;
        }
        history.forEach(item => {
            const div = document.createElement('div');
            div.className = 'history-item p-3 rounded-lg hover:bg-accent cursor-pointer';
    ...
    const updateClearToastsButtonVisibility = () => {
        const toastCount = toastContainer.querySelectorAll(':scope > div.toast').length;
    ...
    window.showToast = (message, options = { type: 'info', details: '', fileName: '' }) => {
        const existingToasts = toastContainer.querySelectorAll(':scope > div.toast');
        ...
        toast.innerHTML = `<div class="w-full rounded-lg border ${bgColor} p-4 shadow-lg">
            <div class="flex items-start">
                ...
                <div class="ml-3 flex-1">
                    ...
                    ${options.details ? `<p class="mt-1 text-sm text-muted-foreground">${options.details}</p>` : ''}
                    ${options.type === 'success' ? `<div class="mt-2"><button data-filename="${options.fileName}" class="retry-download-link text-sm font-medium text-primary hover:underline">Try again</button></div>` : ''}
                </div>
    ...
    window.updateUIForLogin = (user) => {
        if (user) {
            ...
            document.getElementById('user-initials').textContent = displayUsername.charAt(0).toUpperCase();
    ...
    logoutConfirmBackdrop.addEventListener('click', window.hideLogoutConfirmModal);
    ...
    clearToastsButton.addEventListener('click', () => {
        toastContainer.querySelectorAll(':scope > div.toast').forEach(toast => removeToast(toast));
    });
